...
void Sol2ScriptingEnvironment::InitContext(LuaScriptingContext &context)
{
    ...
    // Keep in mind .location is available only if .pbf is preprocessed to set the location with the ref using osmium command "osmium add-locations-to-ways"
    context.state.new_usertype<osmium::NodeRef>("NodeRef",
                                                "id",
                                                &osmium::NodeRef::ref,
                                                "location",
                                                +[](const osmium::NodeRef& nref){ return nref.location(); });
